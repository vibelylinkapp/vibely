-- =====================================================================
-- 0035_seed_kenyan_members.sql
--
-- Ten sample members for demos and screenshots, spread across Kenya
-- rather than all sitting in Nairobi.
--
-- WHY A MIGRATION AND NOT AN INSERT:
--   public.profiles.id references auth.users(id) on delete cascade, so a
--   profile cannot exist without a real auth user. These rows therefore
--   have to be created here rather than through the table API.
--
-- SAFETY:
--   Every account is tagged raw_user_meta_data->>'seed' = 'true' and every
--   profile is is_demo = true, which is exactly what purge_demo_content()
--   looks for. One call removes all ten:
--
--       select public.purge_demo_content();
--
--   The UUIDs are fixed and every insert is ON CONFLICT DO NOTHING, so
--   running this twice is harmless.
--
--   These accounts have no usable password and are not confirmed for
--   sign-in; they exist to populate discovery surfaces only.
--
-- Run in the Supabase SQL Editor after 0034.
-- =====================================================================
...
begin
  ...
  loop
    -- The auth account. Tagged so purge_demo_content() sweeps it up.
    insert into auth.users (
      ...
      rec.id, 'authenticated', 'authenticated',
      rec.handle || '@seed.vibely.local',
      '',                      -- no usable password; these cannot sign in
      now(), now(), now(),
    ...
    insert into public.profiles (
      id, handle, display_name, bio, birthdate, gender, occupation, education,
      languages, height_cm, county, area, avatar_url, cover_url,
      verification, is_online, last_active_at, onboarding_done, is_demo
    ) values (
      rec.id, rec.handle, rec.display_name, rec.bio, rec.birthdate, rec.gender,
      rec.occupation, rec.education,
      array['English','Swahili'], rec.height_cm, rec.county, rec.area,
      '/seed/' || rec.handle || '.jpg',
      'https://picsum.photos/seed/' || rec.handle || '-cover/1000/600',
      rec.verification, rec.is_online,
      now() - (floor(random() * 240) || ' minutes')::interval,
      true, true
    )
    on conflict (id) do nothing;
...
-- Verify:
--   select display_name, county, area, avatar_url from public.profiles where is_demo;
